...
const ProjectCards = ({image,title,description,techstack,link}) => {
  return (
      <div>
        <div onClick={() => window.open(link, "_blank")} className="relative group bg-gradient-to-br from-indigo-500  to-black shadow-lg hover:shadow-2xl hover:shadow-indigo-300 transition-all duration-500 overflow-hidden transform hover:-translate-y-2 w-full max-w-sm">
        ...
        <div className="p-5 text-white">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-semibold tracking-wide">{title}</h2>
              <button className="text-xs px-3 py-1 rounded-md font-bold flex items-center text-white bg-linear-to-br from-green-500"
                  target="_blank"        
